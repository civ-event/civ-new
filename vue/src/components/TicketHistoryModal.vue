  ...
  <Transition name="history-modal-fade">
    ...
    >
      <div class="history-modal__panel">
        <h2 id="ticket-history-title" class="history-modal__title">{{ t('modal.ticketTitle') }}</h2>
          ...
          <table class="history-modal__table">
            <thead>
              <tr>
                ...
                <th>{{ t('modal.source') }}</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(record, index) in records" :key="`${record.obtainedAt}-${index}`">
                <td>{{ formatTime(record.obtainedAt) }}</td>
                <td>+{{ record.amount }}</td>
                <td>{{ formatTicketSource(record.source, t) }}</td>
              </tr>
            </tbody>
          </table>
        </div>
        <p v-else class="history-modal__empty">{{ t('modal.ticketEmpty') }}</p>
...
import { formatTicketSource } from '../utils/wheelHistory';

const { t, locale } = useI18n();
const uiStore = useUiStore();
const wheelStore = useWheelStore();
const { ticketHistoryModalVisible: visible } = storeToRefs(uiStore);
const { ticketHistory: records } = storeToRefs(wheelStore);
